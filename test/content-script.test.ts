  ...
  const onAsk = (event: any) => {
    ...
    window.dispatchEvent(
      new window.MessageEvent('message', {
        data: { source: 'clf-fiber-reply', nonce: event.data.nonce, scanToken, v: 15, scanOk: true, rows, turns: indexedTurns },
        source: window
      })
    );
  };
...
describe('ordinary Continue native recovery', () => {
  ...
  it.each(['reader-only', 'paired'] as const)('recovers a Continue blocked by a cached recorder protocol (%s repair)', async repair => {
    ...
    win.eval(contentSource.replace('const RECORDER_VERSION = 15;', 'const RECORDER_VERSION = 13;')
      .replace('const FIBER_VERSION = 15;', 'const FIBER_VERSION = 12;'));
    await settle();
    ...
    win.postMessage = (data: any, origin: string) => {
      ...
      queueMicrotask(() => {
        ...
        win.dispatchEvent(new win.MessageEvent('message', { source: win, data: {
          source: 'clf-fiber-reply', nonce: data.nonce, scanToken: data.nonce, v: 15, scanOk: true, rows: [], turns: [{
